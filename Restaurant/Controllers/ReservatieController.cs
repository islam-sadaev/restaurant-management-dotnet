...
namespace Restaurant.Controllers
{
    public class ReservatieController : Controller
    {
        ...
        public ReservatieController(IUnitOfWork context, IMapper mapper, IEmailSender emailSender, UserManager<CustomUser> usermanager)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task<IActionResult> TafelReserveren(KlantTafelReserverenViewModel viewModel)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (resMetTijd.Reservaties.Count() > 0) // kijk of dat er reservatie zijn
                {
                    ...
                    if (count >= totaal) // totaal reserver plaatsen zijn grooter of gelijk dan totaal plaatsen
                    {
                        plaats = false;
                    }
                }
        ...
        public async Task<ActionResult> ReservatiesBeheren()
        {
            // Haal enkel reservaties op waarvan de datum >= vandaag is
            var reservaties = await _context.ReservatiesRepository
                .GetAllReservatieMetTijdSlotAsync();

            var toekomstReservaties = reservaties
                .Where(r => r.Datum.HasValue && r.Datum.Value.Date >= DateTime.Today)
                .OrderBy(r => r.Datum.Value)
                .ToList();

            var model = new ReservatieMetTijdSlotListViewModel
            {
                Reservaties = _mapper.Map<List<ReservatieMetTijdSlotViewModel>>(toekomstReservaties)
            };

            return View(model);
        }

        public async Task<IActionResult> Search(ReservatieMetTijdSlotListViewModel vm)
        {
            ReservatieMetTijdSlotListViewModel model = new ReservatieMetTijdSlotListViewModel();

            if (vm.DatumVan.HasValue)
            {
                var reservaties = await _context.ReservatiesRepository.Find(r => r.Datum.Value.Date == vm.DatumVan.Value.Date);
                model.Reservaties = _mapper.Map<List<ReservatieMetTijdSlotViewModel>>(reservaties);
                foreach (var res in model.Reservaties)
                {
                    var tijdslot = await _context.TijdslotRepository.GetByIdAsync(res.TijdSlotId);
                    res.Naam = tijdslot.Naam;
                }
            }
            else
            {
            ...
            return View("ReservatiesBeheren", model);
        }

        public async Task<IActionResult> ReservatieEdit(int id)
        {
            var reservatie = await _context.ReservatiesRepository.GetByIdReservatieMetTijdSlotAsync(id);
            if (reservatie != null)
            {
                var tijdslotenVanuitDatabase = await _context.TijdslotRepository.GetAllAsync();
                ReservatieEditViewModel viewModel = _mapper.Map<ReservatieEditViewModel>(reservatie);
                viewModel.BeschikbareTijdsloten = tijdslotenVanuitDatabase.Select(ts => new SelectListItem
                {
                    Value = ts.Id.ToString(),
                    Text = ts.Naam
            ...
            {
                return RedirectToAction("ReservatiesBeheren");
            }
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> ReservatieEdit(int id, ReservatieEditViewModel viewModel)
        {
            if (id != viewModel.Id)
            {
                ModelState.AddModelError("", "De ReservatieId's komen niet overeen.");
                return View(viewModel);
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                bool plaats = true;
                var waarde = await _context.ParameterRepository.GetByIdAsync(2); //totaal plaatsen in resaurten (50)
                int totaal = int.Parse(waarde.Waarde);
                var resAantal = reservatie.AantalPersonen; //aantal van persoon die nu zijn aan reserverd
                resAantal += (reservatie.AantalPersonen % 2 == 0) ? reservatie.AantalPersonen : reservatie.AantalPersonen + 1;
                if (resAantal == 0)
                ...
                if (plaats)
                {
                    return RedirectToAction("ReservatieBevestiginsOverzicht", reservatie);
                }
                else
                {
                    ModelState.AddModelError("", "Er is geen plek op deze moment");
        ...
        public async Task<IActionResult> ReservatieBevestiginsOverzicht(Reservatie reservatie)
        {
            BevestigingsOverzichtMetIDViewModel vm = _mapper.Map<BevestigingsOverzichtMetIDViewModel>(reservatie);
            var tijdslot = await _context.TijdslotRepository.GetByIdAsync(vm.TijdSlotId);
            vm.Tijdslot = tijdslot.Naam;
        ...
        [HttpPost]
        public async Task<IActionResult> ReservatieBevestiginsOverzicht(BevestigingsOverzichtMetIDViewModel vm)
        {
            var reservatie = await _context.ReservatiesRepository.GetByIdAsync(vm.Id);
        ...
        public async Task<IActionResult> ReservatieDetails(int id)
        {
            Reservatie? reservatie = await _context.ReservatiesRepository.GetByIdReservatieMetTijdSlotAsync(id);
            if (reservatie != null)
            {
                ReservatieDetailsViewModel vm = _mapper.Map<ReservatieDetailsViewModel>(reservatie);
                var tijdslot = await _context.TijdslotRepository.GetByIdAsync(vm.TijdSlotId);
                vm.Tijdslot = tijdslot.Naam;
                return View(vm);
            }
        ...
        public async Task<IActionResult> ReviewIngeven(int reservatieID)
        {
            VMReviewIngeven viewmodel = new VMReviewIngeven();
            viewmodel.reservatie = await _context.ReservatiesRepository.GetByIdAsync(reservatieID);
         
            return View(viewmodel); 
        }
        [HttpPost]
        public async Task<IActionResult> ReviewIngeven(VMReviewIngeven vMReviewIngeven)
        {
            vMReviewIngeven.reservatie.EvaluatieOpmerkingen = vMReviewIngeven.Opmerking;
            vMReviewIngeven.reservatie.EvaluatieAantalSterren = vMReviewIngeven.Score;
            _context.ReservatiesRepository.Update(vMReviewIngeven.reservatie);
            _context.SaveChanges();
